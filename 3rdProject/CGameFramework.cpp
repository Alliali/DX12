...
bool CGameFramework::OnCreate(HINSTANCE hInstance, HWND hMainWnd)
{
	...
	CreateSwapChain();	//또 순서가 밀리는 스왑체인
...
void CGameFramework::CreateSwapChain()
{
	...
	//전체화면 모드에서 바탕홤녀의 해상도를 스왑체인(후면버퍼)의 크기에 맞게 변경한다.
...
void CGameFramework::CreateDirect3DDevice()
{
	...
	m_nFenceValues[m_nSwapChainBuffers] = 0;
...
void CGameFramework::BuildObjects()
{
	...
	//Player에서 카메라를 이용하므로 프레임워크에서 카메라 빌드 삭제
...
//씬 객체를 생성하고 씬에 포함될 게임 객체들을 생성한다.
	...
	//Airplane을 Player로 설정 및 카메라가져오기
	//CAirplanePlayer* pAirplanePlayer = new CAirplanePlayer(m_pd3dDevice, m_pd3dCommandList, m_pScene->GetGraphicsRootSignature()); 따라하기 15 삭제
	//m_pPlayer = pAirplanePlayer; 따라하기 15 변경
...
/*키보드의 상태 정보를 반환한다. 화살표 키(‘→’, ‘←’, ‘↑’, ‘↓’)를 누르면 플레이어를 오른쪽/왼쪽(로컬 x-축), 앞/뒤(로컬 z-축)로 이동한다. ‘Page Up’과 ‘Page Down’ 키를 누르면 플레이어를 위/아래(로컬 y-축)로 이동한다.*/
	...
	POINT ptCursorPos;
/*마우스를 캡쳐했으면 마우스가 얼마만큼 이동하였는 가를 계산한다. 마우스 왼쪽 또는 오른쪽 버튼이 눌러질 때의 메시지(WM_LBUTTONDOWN, WM_RBUTTONDOWN)를 처리할 때 마우스를 캡쳐하였다. 그러므로 마우스가 캡쳐된 것은 마우스 버튼이 눌려진 상태를 의미한다. 마우스 버튼이 눌려진 상태에서 마우스를 좌우 또는 상하로 움직이면 플레이어를 x-축 또는 y-축으로 회전한다.*/
...
/*플레이어를 dwDirection 방향으로 이동한다(실제로는 속도 벡터를 변경한다). 이동 거리는 시간에 비례하도록 한다. 플레이어의 이동 속력은 (50/초)로 가정한다.*/
		if (dwDirection) m_pPlayer->Move(dwDirection, 250.0f * m_GameTimer.GetTimeElapsed(), true);
	}
...
void CGameFramework::FrameAdvance()
{
	...
	//Descriptor들이 연속적인 범위로 할때 TRUE 아니면 FALSE, 현재 서술자가 하나라 1
	...
	D3D12_CPU_DESCRIPTOR_HANDLE d3dDsvCPUDescriptorHandle = 
	m_pd3dDsvDescriptorHeap->GetCPUDescriptorHandleForHeapStart();
	//깊이-스텐실 서술자의 CPU 주소를 계산한다. 다시 내리기
...
//3인칭 카메라일 때 플레이어를 렌더링한다. 
	...
	/*현재의 프레임 레이트를 문자열로 가져와서 주 윈도우의 타이틀로 출력한다. m_pszBuffer 문자열이 "LapProject ("으로 초기화되었으므로 (m_pszFrameRate+12)에서부터 프레임 레이트를 문자열로 출력하여 " FPS)" 문자열과 합친다.
...
{
	UINT64 nFenceValue = ++m_nFenceValues[m_nSwapChainBufferIndex];	//m_nFenceValue++; <- 동기화 시켜야 하므로 수정
	HRESULT hResult = m_pd3dCommandQueue->Signal(m_pd3dFence, nFenceValue);
	//GPU가 펜스의 값을 설정하는 명령을 명령 큐에 추가한다. 지정한 값으로 펜스 설정
	if (m_pd3dFence->GetCompletedValue() < nFenceValue)
	{
		//펜스의 현재 값이 설정한 값보다 작으면 펜스의 현재 값이 설정한 값이 될 때까지 기다린다. 
		hResult = m_pd3dFence->SetEventOnCompletion(nFenceValue, m_hFenceEvent);	// 값에 도달하면 이벤트 발생
		::WaitForSingleObject(m_hFenceEvent, INFINITE); //윈도우함수로 이벤트 기다리기 무한정
	}
}
