...
{
	//XMStoreFloat4x4(&m_xmf4x4World, XMMatrixIdentity()); 따라하기 15 변경
...
//정점 버퍼를 위한 업로드 버퍼를 소멸시킨다.
	//if (m_pMesh) m_pMesh->ReleaseUploadBuffers(); 따라하기 15 변경
	if (m_ppMeshes)
	{
		for (int i = 0; i < m_nMeshes; i++)
		{
...
//게임 객체에 셰이더 객체가 연결되어 있으면 셰이더 상태 객체를 설정한다.
	if (m_pShader) { 
		...
		//셰이더가 아닌 게임오브젝트에서 상수버퍼갱신을 진행
		m_pShader->Render(pd3dCommandList, pCamera);
		//m_pShader->Render(pd3dCommandList); 
	}

//게임 객체에 메쉬가 연결되어 있으면 메쉬를 렌더링한다.
	//if (m_pMesh) m_pMesh->Render(pd3dCommandList); 따라하기 15 변경
...
CRotatingObject::CRotatingObject(int nMeshes) : CGameObject(nMeshes) // 따라하기 15 변경
{
	m_xmf3RotationAxis = XMFLOAT3(0.0f, 1.0f, 0.0f);
	m_fRotationSpeed = 15.0f;	// 속도 90에서 15로 변경
}
